...
parser.add_argument("--store-bets", dest="store_bets", default=False, action="store_true", help="store simulated bets")
...
if args.simulate:
    # for bet in bets: APP.simulate(bet, args.store_bets)
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        [executor.submit(APP.simulate, bet, args.store_bets) for bet in bets]
